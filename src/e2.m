...
% Amplitudes des 3 signaux
A1 = 10;  % Amplitude du signal à 10 Hz
A2 = 1;   % Amplitude du signal à 500 Hz
A3 = 1;   % Amplitude du signal à 1000 Hz
fprintf('Amplitudes : A1 = %d, A2 = %d, A3 = %d\n\n', A1, A2, A3);

% Génération des 3 signaux sinusoïdaux avec amplitudes différentes
S1 = A1 * sin(2*pi*f1*t);  % Signal à 10 Hz, amplitude 10
S2 = A2 * sin(2*pi*f2*t);  % Signal à 500 Hz, amplitude 1
S3 = A3 * sin(2*pi*f3*t);  % Signal à 1000 Hz, amplitude 1
...
xlabel('Temps (s)');
ylabel('Amplitude');
title('Signal temporel : S(t) = 10×sin(2\pi×10t) + sin(2\pi×500t) + sin(2\pi×1000t)');
grid on;
xlim([0 0.1]);  % Afficher les 100 premières millisecondes
ylim([-12.5 12.5]);  % Adapter à la nouvelle amplitude maximale (~12)
